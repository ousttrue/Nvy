            ...
            for (size_t j = 0; j < cells_array_length; ++j)
            {
                ...
                for (int k = 0; k < repeat; ++k)
                {
                    ...
                    wstrlen = MultiByteToWideChar(
                        CP_UTF8, 0, str.data(), str.size(), &grid->Chars()[idx],
                        grid_size - idx);
                }
...
public:
                ...
                const wchar_t *window_title)
    {
        ...
        _hwnd = CreateWindowEx(WS_EX_ACCEPTFILES, _class_name.c_str(),
                    ...
                    PWSTR p_cmd_line, int n_cmd_show)
{
    ...
    Win32Window window;
    auto hwnd = window.Create(instance, L"Nvy_Class", L"Nvy");
    if (!hwnd)
    {
        return 1;
    }
...

    {
        ...
        rpc.attach(msgpackpp::WindowsPipeTransport(
            io_context, nvim.ReadHandle(), nvim.WriteHandle()));
        ...
        {
            auto result =
                rpc.request_async("nvim_eval", "stdpath('config')").get();
            ...
            // void SendUIAttach(int grid_rows, int grid_cols)
            {
                ...
                auto msg = msgpackpp::make_rpc_notify_packed(
                    "nvim_ui_attach", args.get_payload());
                rpc.write_async(msg);
            }
