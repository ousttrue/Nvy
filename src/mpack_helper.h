...
inline void MPackStartRequest(int64_t msg_id, const char *request,
...
inline void MPackStartNotification(const char *notification,
                                   mpack_writer_t *writer)
{
    mpack_start_array(writer, 3);
    mpack_write_i64(writer,
                    static_cast<int64_t>(MPackMessageType::Notification));
...
inline MPackMessageResult MPackExtractMessageResult(mpack_tree_t *tree)
{
    ...
    MPackMessageType message_type = static_cast<MPackMessageType>(
        mpack_node_array_at(root, 0).data->value.i);
    if (message_type == MPackMessageType::Request)
    {
        ...
        return MPackMessageResult{
            ...
            .request = {.method = mpack_node_array_at(root, 2),
    ...
    else if (message_type == MPackMessageType::Response)
    {
        ...
        return MPackMessageResult{
            .type = message_type,
            .params = mpack_node_array_at(root, 3),
            .response{.error = mpack_node_array_at(root, 2),
                      .msg_id = mpack_node_array_at(root, 1).data->value.i}};
    }
    else if (message_type == MPackMessageType::Notification)
    {
        ...
        return MPackMessageResult{
            ...
            .notification = {.name = mpack_node_array_at(root, 1)}};
    }
