...
DWORD WINAPI NvimMessageHandler(LPVOID param)
{
    ...
    mpack_tree_init_stream(tree, ReadFromNvim, nvim->stdout_read, Megabytes(20),
                           1024 * 1024);
    ...
    while (true)
    {
        ...
        SendMessage(nvim->hwnd, WM_NVIM_MESSAGE, reinterpret_cast<WPARAM>(tree),
                    0);
    }
...
void NvimSendModifiedInput(Nvim *nvim, const char *input, bool virtual_key)
{
    ...
    mpack_writer_t writer;
    mpack_writer_init(&writer, data, MAX_MPACK_OUTBOUND_MESSAGE_SIZE);
    MPackStartRequest(RegisterRequest(nvim, nvim_input),
                        ...
                        int mouse_row, int mouse_col)
{
    ...
    mpack_writer_t writer;
    mpack_writer_init(&writer, data, MAX_MPACK_OUTBOUND_MESSAGE_SIZE);
    MPackStartRequest(RegisterRequest(nvim, nvim_input_mouse),
...
void NvimOpenFile(Nvim *nvim, const wchar_t *file_name)
{
    ...
    mpack_writer_t writer;
    mpack_writer_init(&writer, data, MAX_MPACK_OUTBOUND_MESSAGE_SIZE);
    MPackStartRequest(RegisterRequest(nvim, nvim_command),
