...
static DWORD WINAPI NvimMessageHandler(LPVOID param)
{
    ...
    while (true)
    {
        ...
        mpack_tree_init_stream(tree.get(), ReadFromNvim, nvim->stdout_read,
...
bool NvimPipe::Launch(const wchar_t *command_line)
{
    SECURITY_ATTRIBUTES sec_attribs{.nLength = sizeof(SECURITY_ATTRIBUTES),
                                    .bInheritHandle = true};
    if (!CreatePipe(&this->stdin_read, &this->stdin_write, &sec_attribs, 0))
    {
        return false;
    }
    if (!CreatePipe(&this->stdout_read, &this->stdout_write, &sec_attribs, 0))
    {
        return false;
    }
    ...
    if (!CreateProcessW(nullptr, std::wstring(command_line).data(), nullptr,
                        ...
                        &startup_info, &this->process_info))
    {
        return false;
    }
